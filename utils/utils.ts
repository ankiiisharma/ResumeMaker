...
import { Education, PersonalDetails } from '@/types/backend';


const writeFilePromise = promisify(fs.writeFile);

//personal detail
const personalDetailCreator = (data: PersonalDetails) => {
...
const educationCreator = (data: Education) => {
    if (data == null) {
        return ''
    }

    let info = ``
    data.map((val: Education) => {
